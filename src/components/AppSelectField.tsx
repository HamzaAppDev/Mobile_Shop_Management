...
type CommonProps<T> = {
    ...
    // Optional custom row
    renderItem?: (args: { item: T; selected: boolean; onPress: () => void }) => React.ReactNode;
...
export type AppSelectFieldProps<T> = CommonProps<T> & (SingleSelectProps<T> | MultiSelectProps<T>);

function defaultFilter<T>(getLabel: (item: T) => string, item: T, query: string) {
...
function AppSelectFieldBase<T>(props: AppSelectFieldProps<T>) {
    ...
    const selectedValue: Id | null = !isMultiple ? ((singleIsControlled ? props.value : internalValue) ?? null) : null;

    const selectedValues: Id[] = isMultiple ? (multiIsControlled ? (props.values ?? []) : internalValues) : [];
    ...
    const selectedItem = useMemo(() => {
        ...
        const found = data.find((it) => toKey(getValue(it)) === toKey(selectedValue));
        return found ?? null;
    ...
    }, [getLabel, isMultiple, maxPreviewLabels, selectedItem, selectedItemsMulti]);
    ...
    const handleSelect = useCallback(
        (item: T) => {
            ...
            if (isMultiple) {
                ...
                const closeOnPick = typeof props.closeOnSelect === "boolean" ? props.closeOnSelect : false;
            ...
            const closeOnPick = typeof props.closeOnSelect === "boolean" ? props.closeOnSelect : true;
            if (closeOnPick) closeSheet();
        },
        [closeSheet, commitMulti, commitSingle, getValue, isMultiple, props.closeOnSelect, selectedValues]
    );
    ...
    const selectedIndexInFiltered = useMemo(() => {
        ...
        return filtered.findIndex((it) => toKey(getValue(it)) === toKey(selectedValue));
    }, [filtered, getValue, isMultiple, scrollToSelectedOnOpen, selectedValue, selectedValues]);
    ...
    const onScrollToIndexFailed = useCallback(
        (info: { index: number; averageItemLength: number }) => {
            const offset = info.index * (info.averageItemLength || estimatedItemHeight);
    ...
    const keyExtractor = useCallback((item: T) => String(getValue(item)), [getValue]);
    ...
    const renderRow = useCallback(
        ({ item }: ListRenderItemInfo<T>) => {
            ...
            return (
                ...
                >
                    <AppText style={{ color: colors.text, fontWeight: selected ? "700" : "600", flex: 1 }}>
                        {getLabel(item)}
                    </AppText>

                    {/* lightweight check indicator */}
                    {selected ? (
                        <AppText style={{ color: colors.primary, fontWeight: "800", marginLeft: 10 }}>✓</AppText>
                    ) : null}
                </Pressable>
            );
        },
        [colors.primary, colors.text, getLabel, handleSelect, isSelected, mode, renderItem]
    );
    ...
    return (
        <View style={[containerStyle, { width: "100%" }]}>
            ...
            >
                ...
                <AppText style={{ color: colors.muted, fontSize: 18, fontWeight: "700" }}>▾</AppText>
            </Pressable>
...
export const AppSelectField = memo(AppSelectFieldBase) as typeof AppSelectFieldBase;
